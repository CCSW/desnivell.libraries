    ...
    public partial class SitemapGenerator : BaseSitemapGenerator, ISitemapGenerator
    {
        ...
        private void WriteCategories(int parentCategoryId)
        {
            ...
            foreach (var category in categories)
            {
                ...
                var url = string.Format("{0}{2}-c-{1}", _webHelper.GetStoreLocation(false), category.Id, category.GetSeName());
        ...
        private void WriteManufacturers()
        {
            ...
            {
                //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
                var url = string.Format("{0}{2}-m-{1}", _webHelper.GetStoreLocation(false), manufacturer.Id, manufacturer.GetSeName());
                var updateFrequency = UpdateFrequency.Weekly;
        ...
        private void WriteProducts()
        {
            ...
            {
                //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
                var url = string.Format("{0}{2}-p-{1}", _webHelper.GetStoreLocation(false), product.Id, product.GetSeName());
                var updateFrequency = UpdateFrequency.Weekly;
        ...
        private void WriteTopics()
        {
            ...
            foreach (var topic in topics)
            {
                ...
                var url = string.Format("{1}", _webHelper.GetStoreLocation(false), topic.SystemName.ToLowerInvariant());
                var updateFrequency = UpdateFrequency.Weekly;
